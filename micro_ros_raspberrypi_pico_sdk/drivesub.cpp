...
void joy_callback(const void* msgin)
{
  ...
  float leftSpeed  = msg->axes.data[1];
  float rightSpeed = msg->axes.data[4];
  ...
  rcl_publish(&publisher, &("L: [%f], R: [%f]", leftSpeed, rightSpeed), NULL);
...
int main()
{
    ...
    RCCHECK(rclc_executor_add_subscription(&executor, &subscriber, &msg, &joy_callback, ON_NEW_DATA));
